...
describe("mcpParityCommand", () => {
  it("supports state write/read aliases via CLI parity", async () => {
    ...
    try {
      ...
      const writeResult = JSON.parse(logs.pop() || "{}") as {
        path?: string;
      };
  ...
  it("supports notepad and project-memory aliases via CLI parity", async () => {
  ...
  it("supports trace summary alias via CLI parity", async () => {
...
describe("executeMcpParityCommand", () => {
  it("returns command-local help text", async () => {
    const result = await executeMcpParityCommand("trace", ["help"]);
    assert.equal(result.ok, true);
    assert.match("help" in result ? result.help : "", /Usage: omx trace/);
  });
});
